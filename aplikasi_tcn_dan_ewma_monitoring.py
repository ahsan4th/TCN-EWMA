# -*- coding: utf-8 -*-
"""Aplikasi Streamlit untuk TCN dan EWMA Monitoring

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1zRhvnu9fQUqMM82DnHe2dtY_Q4DzNTAH
"""
...
def calculate_ewma(residuals, lambda_param, L):
    ...
    Args:
        residuals (np.array): Residual dari model forecasting.
    ...
    # Untuk n kecil, ada faktor koreksi, tapi untuk kesederhanaan kita gunakan ini.
    UCL = L * sigma_residuals * np.sqrt(lambda_param / (2 - lambda_param))
    ...
    try:
        ...
        if 'Tanggal' in df.columns and 'Volume' in df.columns:
            ...
            if st.sidebar.button("Jalankan Analisis"):
                ...
                test_data = scaled_data[train_size - look_back:, :] # Pastikan test data memiliki look_back
                ...
                # Buat indeks untuk prediksi training dan testing
                train_predict_index = df_volume.index[look_back : look_back + len(train_predict)]
                test_predict_index = df_volume.index[train_size + look_back : train_size + look_back + len(test_predict)]

                ax_forecast.plot(train_predict_index, train_predict, label='Prediksi Training TCN', color='blue')
                ax_forecast.plot(test_predict_index, test_predict, label='Prediksi Testing TCN', color='red')
                ...
                residuals_df = pd.DataFrame(residuals, index=test_predict_index, columns=['Residual'])
